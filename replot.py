...
for image in os.scandir(folder):
	if image.is_dir():
		continue
	reports, maxP, _ = np.load(image.path.replace("img", "data/img").replace(".png", "_all.npy"), allow_pickle=True) # _ = fileName
	...
	# datTime = - 1645000000 + os.path.getmtime(image.path.replace("img", "data/img").replace(".png", "_all.npy"))
	...
	# # 	print(image.path, image.path.replace("img", "data/img").replace(".png", "_all.npy"))
	# # print()
...
# 	if image.is_dir():
# 		continue
# 	if not (os.path.exists(image.path.replace("img", "data/img").replace(".png", ".npy")) and os.path.exists(image.path.replace("img", "data/img").replace(".png", "_sol.npy"))):
# 		# print(f"File {image.name.replace('.png', '_all.npy')} doesn't exist")
# 		continue
	
# 	print("Actually doing something")
# 	states, rates = np.load(image.path.replace("img", "data/img").replace(".png",     ".npy"), allow_pickle=True)
# 	topStat, minE = np.load(image.path.replace("img", "data/img").replace(".png", "_sol.npy"), allow_pickle=True)
	
# 	counts = {k:v for k,v in zip(states, rates)}


# 	# imgTime = - 1645000000 + os.path.getmtime(image.path)
# 	# datTime = - 1645000000 + os.path.getmtime(image.path.replace("img", "data/img").replace(".png", ".npy"))
# 	# datTime = - 1645000000 + os.path.getmtime(image.path.replace("img", "data/img").replace(".png", "_sol.npy"))
